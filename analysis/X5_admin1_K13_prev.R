...
# Convert k13_prev_data to an sf object
k13_prev_sf <- st_as_sf(k13_prev_data, coords = c("longitude", "latitude"), crs = 4326)

# Ensure both layers have the same CRS
africa_shp_admin1 <- st_transform(africa_shp_admin1, crs = st_crs(k13_prev_sf))

# Spatial join K13 prev data with admin1
k13_prev_data_with_admin1 <- st_join(k13_prev_sf, africa_shp_admin1, join = st_within)
# Identify unmatched rows
unmatched <- k13_prev_sf[is.na(k13_prev_data_with_admin1$iso), ]
# Perform fuzzy join (e.g., 10 km buffer) on only the unmatched rows
dist_threshold <- 10000
# Fuzzy spatial join: points within distance to polygon
fuzzy_matched <- st_join(
  unmatched,
  africa_shp_admin1,
...
# Combine both sets
k13_prev_data_with_admin1_final <- rbind(matched_only, fuzzy_matched)

# Check for how many points there is no match
k13_prev_data_with_admin1 %>%
  filter(is.na(iso)) %>%
  ggplot() +
  geom_sf(color = "red") +
...
africa_map_points <- ggplot() +
  facet_wrap(~year) +
  geom_sf(data = africa_shp_admin0, fill = NA, color = "black", show.legend = FALSE, lwd = 0.1) +
  geom_point(data = filter(k13_prev_data, k13_prevalence == 0),
...
inset_data <- k13_prev_data %>% filter(
