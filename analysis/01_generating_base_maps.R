...
adm2_shp_available <- malariaAtlas::listShp(admin_level = "admin2", printed = F) # 27687 adm2
shp_list <- lapply(adm2_shp_available$iso, function(iso) {
  malariaAtlas::getShp(ISO = iso, admin_level = "admin1")
})
shp_adm2 <- do.call(rbind, shp_list)

# Convert to sf object ----------------------------------------------------

sf_adm0 <- shp_adm0 %>% sf::st_as_sf()
sf_adm1 <- shp_adm1 %>% sf::st_as_sf()
sf_adm2 <- shp_adm2 %>% sf::st_as_sf()
...
saveRDS(sf_adm2_africa, "analysis/data_derived/sf_admin2_africa.rds")
